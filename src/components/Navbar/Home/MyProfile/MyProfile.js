...
const MyProfile = () => {
    ...
    const { data: userData, isLoading, refetch } = useQuery('user', () => axios.get(`https://safe-plains-03194.herokuapp.com/user?email=${user?.email}`, {
        headers: {
    ...
    const handleUpdate = async (e) => {
        ...
        await axios.put(`https://safe-plains-03194.herokuapp.com/user-update?email=${user?.email}`, { name, address, phone, linkedin, email: user.email }, {
            headers: {
                authorization: `Bearer ${localStorage.getItem('accessToken')}`
            }
        })
