...
const User = () => {
    const { data: user, isLoading, refetch } = useQuery('all-user', async () => await axios.get('https://safe-plains-03194.herokuapp.com/all-user'))
    ...
    const makeAdmin = async (email, name) => {
        await axios.put(`https://safe-plains-03194.herokuapp.com/user/admin/${email}`).then(response => console.log(response))
        refetch()
    ...
    const handleDeleteUser = async (id) => {
        await axios.delete(`https://safe-plains-03194.herokuapp.com/user/${id}`)
        refetch()
