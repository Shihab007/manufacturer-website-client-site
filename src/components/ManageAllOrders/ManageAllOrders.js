...
const ManageAllOrders = () => {
    ...
    const { data, isLoading, refetch } = useQuery('orders', () => axios.get('https://safe-plains-03194.herokuapp.com/orders'))
    if (isLoading) {
    ...
    const handleShipped = async (id, status) => {
        if (!status) {
            await axios.put(`https://safe-plains-03194.herokuapp.com/orders/${id}`, { status: 'shipped' })
                ...
                <tbody>

                    {
                        orders?.length > 0 && orders.map((order, index) => <tr key={order._id}>
                            ...
                            {
                                !order.payment ? <td><label for="delete-confirm-modal-orders" onClick={() => setOrdersForModal(order)} className='btn btn-color'>Cancel</label></td> : <td><button className='btn btn-outline' disabled>Cancel</button></td>
                            }
