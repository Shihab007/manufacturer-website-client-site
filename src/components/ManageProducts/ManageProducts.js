...
const ManageProducts = () => {
    ...
    const { data, isLoading, refetch } = useQuery("parts", () => axios.get('https://safe-plains-03194.herokuapp.com/parts'))
    if (isLoading) {
                    ...
                    <tbody>

                        {
                            products?.length > 0 && products.map((product, index) => <tr key={product._id}>
                                ...
                                <td>{product.price}</td>
                                <td><label onClick={() => setProduct(product)} for="delete-confirm-modal" className='btn btn-color'>Delete Product</label></td>
                            </tr>
