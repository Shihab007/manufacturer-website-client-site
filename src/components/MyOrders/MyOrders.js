...
const MyOrders = () => {
    ...
    const { data: orders, isLoading, refetch } = useQuery('order', () => axios.get(`https://safe-plains-03194.herokuapp.com/order?email=${email}`))
    if (isLoading) {
        return <Loading></Loading>
    }
    const handleDelete = (id) => {
        axios.delete(`https://safe-plains-03194.herokuapp.com/order/${id}`).then(response => refetch())
    }
                    ...
                    orders.data.map(product => <div key={product._id} className="card w-full  bg-base-100 shadow-xl">
                        ...
                        <div className="card-body">
                            ...
                            <label disabled={product.payment} for="delete-confirm-modal-orders" onClick={() => {
                                ...
                                refetch()
                            }} className='btn btn-color'>Cancel</label>
